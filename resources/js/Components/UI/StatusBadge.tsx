...
export default function StatusBadge({ status, labelOverride }: StatusBadgeProps) {
        ...
        switch (status) {
            ...
            case 'finished':
                return {
                    ...
                    dot: 'bg-emerald-500',
                    label: status === 'paid' ? 'Lunas' : status === 'received' ? 'Diterima' : 'Selesai'
                };
            case 'issued':
            case 'active':
                return {
    ...
    return (
        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold border leading-none ${styles.bg}`}>
