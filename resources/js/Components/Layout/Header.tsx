...
export default function Header({ title, breadcrumbs, fiscalMode, isCollapsed = false, onToggleCollapse }: HeaderProps) {
        ...
        <header className="sticky top-0 z-30 h-16 bg-white/80 backdrop-blur-md border-b border-slate-200/55 px-6 md:px-8 flex items-center justify-between">
            <div className="flex items-center gap-3.5">
                ...
                <button
                    ...
                    className="hidden lg:flex p-2 rounded-xl text-slate-500 hover:text-slate-900 hover:bg-slate-100 border border-slate-200/70 transition-all cursor-pointer shrink-0"
                    title={isCollapsed ? 'Perluas Sidebar' : 'Ciutkan Sidebar'}
                >
                    {isCollapsed ? (
                        <svg className="w-4 h-4 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.2}>
