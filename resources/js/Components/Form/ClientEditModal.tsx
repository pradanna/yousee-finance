...
export default function ClientEditModal({ isOpen, onClose, client, onSubmit }: ClientEditModalProps) {
        ...
        <Modal show={isOpen} onClose={onClose} maxWidth="xl" closeable={false}>
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
                ...
                <div className="flex items-start gap-4 pb-4 border-b border-slate-100">
                    ...
                    <div>
                        <h3 className="text-base font-bold text-slate-800 tracking-tight">Edit Data Client</h3>
                        <p className="text-xs text-slate-500 mt-0.5">
                            Ubah rincian profil & status perpajakan client <span className="font-bold text-slate-700">{client.name}</span>
                        </p>
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <InputLabel htmlFor="edit-client-name" value="Nama Lengkap Client *" />
                            <TextInput
                                id="edit-client-name"
                                type="text"
                                value={form.name}
                        ...
                        <div>
                            <InputLabel htmlFor="edit-client-npwp" value="NPWP Resmi Client" />
                            <TextInput
                                id="edit-client-npwp"
                                type="text"
                                value={form.npwp}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <InputLabel htmlFor="edit-client-email" value="Email Kontak Finance" />
                            <TextInput
                        ...
                        <div>
                            <InputLabel htmlFor="edit-client-phone" value="Telepon / WhatsApp" />
                            <TextInput
                    ...
                    {/* Alamat Lengkap */}
                    <div>
                        <InputLabel htmlFor="edit-client-address" value="Alamat Kantor Client" />
                        <textarea
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                        ...
                        <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex items-start justify-between gap-3">
                            <div className="space-y-1">
                                <span className="text-xs font-bold text-slate-700 block">Status Client</span>
                            ...
                            <button
                                type="button"
                                onClick={() => setForm({ ...form, status: isClientActive ? 'archived' : 'active' })}
                                className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                                    isClientActive ? 'bg-primary' : 'bg-slate-300'
                                }`}
                            >
                                <span
                                    className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow-md ring-0 transition duration-200 ease-in-out ${
                                        isClientActive ? 'translate-x-5' : 'translate-x-0'
                                    }`}
                                />
                            </button>
                        </div>
                        ...
                        <div className="flex items-start gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                            ...
                            <div className="space-y-0.5">
                                <label htmlFor="edit-client-pkp-checkbox" className="text-xs font-bold text-slate-700 cursor-pointer block">
                                    Status PKP (Bisa PPN)
                                </label>
                                <span className="text-[10px] text-slate-400 font-semibold block leading-tight">
