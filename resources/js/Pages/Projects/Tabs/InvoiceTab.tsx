...
export default function InvoiceTab({
    ...
    const handleDownloadKwitansiPdf = (term: PaymentTerm) => {
        const totalAmount = isPPN
            ? Math.round(term.amount * 1.11)
            : term.amount;
        const paidAmount = term.paidAmount
            ? isPPN
                ? Math.round(term.paidAmount * 1.11)
                : term.paidAmount
            : totalAmount;
        ...
        const termIdx =
            (project.clientPaymentPlan?.terms?.findIndex(
                (t) => t.id === term.id,
            ) ?? 0) + 1;
        const receiptNum = term.paymentRef
            ? `KW-${term.paymentRef}`
            : `KW-${project.invoiceNumber || project.code}-${termIdx}`;
                ...
                <div className="flex items-start gap-3 rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4">
                    ...
                    <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 text-xs font-bold text-amber-900">
                            Pengingat Penagihan: {dueAlerts.length} Termin
            ...
            <div className="shadow-xs space-y-6 rounded-3xl border border-slate-200/90 bg-white p-6">
                <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                    <div>
                        <h3 className="flex items-center gap-2 text-xs font-black uppercase tracking-wider text-slate-400">
                            ...
                            Penagihan & Skema Pembayaran Client
                        </h3>
                        <p className="mt-0.5 text-[11px] text-slate-500">
                            Atur skema termin, tanggal jatuh tempo, dan
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center justify-between">
                            <h4 className="text-xs font-bold text-slate-800">
                                Tahapan Penagihan per Termin (
                                {
                                    SCHEME_LABELS[
                                        project.clientPaymentPlan.scheme
                                    ]
                                }
                                )
                            </h4>
                            <span className="text-[10px] font-semibold text-slate-400">
                                Total {project.clientPaymentPlan.terms.length}{' '}
                                Termin Penagihan
                            </span>
                        </div>
                        ...
                        <div className="flex items-center justify-between border-t border-slate-100 pt-4">
                            <div>
                                ...
                                <p className="mt-0.5 text-[10px] text-slate-400">
                                    Catatan log transaksi dana masuk dari client
                                </p>
                            </div>
                            {(() => {
                                const paidTerms =
                            ...
                            if (paidTerms.length === 0) {
                                return (
                                    <div className="rounded-2xl border border-slate-200/60 bg-slate-50/50 p-4 text-center">
                                        ...
                                        <div className="mt-0.5 text-[10px] text-slate-400">
                                            Klik tombol "Terima Pembayaran" pada
                                            termin di atas untuk mencatat
                                            pembayaran masuk
                                        </div>
                                    </div>
                                );
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2 rounded-2xl border border-slate-200/60 bg-slate-50/70 p-4">
                        ...
                        {project.clientPaymentPlan ? (
                            (() => {
                                const plan = project.clientPaymentPlan!;
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-slate-200/80 bg-slate-50 p-4">
                    <div>
                        <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400">
                            Ringkasan Tagihan Akhir Proyek
                        </div>
