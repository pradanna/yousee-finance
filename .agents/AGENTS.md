...
- **Strict Typing Policy (Zero `any` Allowed)**:
    - The use of `any` type in TypeScript is **STRICTLY FORBIDDEN**.
    - Always define explicit interfaces, type aliases, generic types, or use `unknown` with proper type narrowing/type guards if the shape is truly dynamic.
- **Code Formatting Policy (Prettier Standard)**:
    - All frontend and TypeScript/JavaScript code MUST strictly follow the project's Prettier formatting rules (e.g., 4-space indentation, single quotes, trailing commas, line length limits).
    - All files MUST use Unix-style line endings (`LF` / `\n`). Windows line endings (`CRLF`) are strictly forbidden.
- **Language Policy**:
